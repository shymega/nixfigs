        ...
        forSystem (system:
          ...
          in
          {
            ...
            sdImages-collections =
              ...
              with images;
              {
                all = with builtins; map (k: getAttr k self.builds.${system}.sdImages)
                  (attrNames self.builds.${system}.sdImages);
            ...
            isos-collections =
              ...
              with isos;
              {
                all = with builtins; map (k: getAttr k self.builds.${system}.isos)
                  (attrNames self.builds.${system}.isos);
              };
            ...
            all = pkgs.symlinkJoin {
              ...
              paths =
                ...
                in
                with builtins;
                (map (k: getAttr k self.builds.${system}.sdImages)
                  (attrNames self.builds.${system}.sdImages))
                ++ (map (k: getAttr k generatorsAll)
                  (attrNames generatorsAll))
                ++ (map (k: getAttr k self.builds.${system}.isos.all)
                  (attrNames self.builds.${system}.isos.all));
            };
          });
      common = inputs.nixfigs-common.common // inputs.nixfigs-private.common;
      inherit (inputs.nixfigs-helpers) helpers;
      homeConfigurations = inputs.nixfigs-homes.homeConfigurations // inputs.nixfigs-private.homeConfigurations;
      inherit (inputs.nixfigs-homes) homeModules; # FIXME: Add `nixfigs-private.homeModules`.
      inherit (inputs.nixfigs-networks) networks;
      nixosConfigurations = inputs.nixfigs-private.nixosConfigurations // inputs.nixfigs-public.nixosConfigurations;
