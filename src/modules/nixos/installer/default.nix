...
let
  ...
  inherit (config.nixpkgs.hostPlatform) isAarch64;
  ...
  outputFormat =
    if hasIsoModule && hasSdModule then
      "both"
    else if hasIsoModule then
      "iso"
    else if hasSdModule then
      "sd-card"
    else
      "iso"; # fallback
  ...
  config = mkIf cfg.enable (mkMerge [
    {
      ...
      users.users.installer = {
        ...
        openssh.authorizedKeys.keyFiles =
          mkIf (cfg.sshKeys == [ ] && (config.sops.secrets ? "installer-ssh-keys"))
            [
    ...
    (mkIf hasSdModule {
      sdImage = {
        ...
        inherit (cfg.sdCard) populateRootCommands;
        inherit (cfg.sdCard) populateFirmwareCommands;
        inherit (cfg.sdCard) firmwareSize;
